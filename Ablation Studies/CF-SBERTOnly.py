...
def train_and_evaluate(training_data, testing_data, items_dict):
    ...
    for epoch in range(num_epochs):
        ...
        print(user_out[user_indices].float())
        predicted_ratings = model.lin(user_out[user_indices].float()).squeeze()
    ...
    with torch.no_grad():
        ...
        predicted_ratings = model.lin(user_out[user_indices].float()).squeeze().tolist()
...
if __name__ == '__main__':
    ...
    threshold_for_fashion = [5]
    threshold_for_beauty = [5]
    threshold_for_movie = [5]
    threshold_for_music = [5]
    threshold_for_toys = [5]
    ...
    df_path_list = [beauty_path, fashion_path, movie_path, music_path, toys_path]
    meta_df_path_list = [meta_beauty_path, meta_fashion_path, meta_movie_path, meta_music_path, meta_toys_path]
    threshold_list = [threshold_for_beauty, threshold_for_fashion, threshold_for_movie, threshold_for_music,
                      threshold_for_toys]
    ...
    for df_path, meta_df_path, threshold in zip(df_path_list, meta_df_path_list, threshold_list):
        ...
        if 'beauty' in df_path:
            output_path = f'../../Datasets/SBERT_hidden_channels={hidden_channels}_{args.rec_pre_trained_data}.xlsx'
        else:
            output_path = f'../../Datasets/SBERT_hidden_channels={hidden_channels}_{args.rec_pre_trained_data}.xlsx'
    ...
    '''
    Dataset: ../../Datasets/beauty.pkl RMSE: 1.4088291585323538 MAE: 1.0263570542480598 Hidden channels: 128 threshold: 5
    [1.4196999572403428, 1.437990688167492, 1.420790387036136, 1.3635832513374242, 1.4020815088803746]
    [1.0294996516382806, 1.0331793130235647, 1.0291687002846235, 1.008734538389338, 1.031203067904491]
    
    Dataset: ../../Datasets/fashion.pkl RMSE: 1.5391832163677157 MAE: 1.0792997001245839 Hidden channels: 128 threshold: 5
    [1.5267635182707586, 1.5569066945708303, 1.5255294772420303, 1.5338192273840519, 1.5528971643709066]
    [1.076636318769985, 1.0857393378568039, 1.0764420469247076, 1.076037048932151, 1.081643748139272]
    
    Dataset: ../../Datasets/Musical_Instruments.pkl RMSE: 1.291990663745727 MAE: 0.9644509939560214 Hidden channels: 128 threshold: 5
    [1.2946785100572122, 1.2941877420411607, 1.2946245865148092, 1.2905103708424568, 1.2859521092729973]
    [0.9652602440339091, 0.9659472991512029, 0.966864685206372, 0.9621075758702303, 0.9620751655183921]
    
    
    Dataset: ../../Datasets/Movies_and_TV.pkl RMSE: 1.2154354837420758 MAE: 0.9243540551580557 Hidden channels: 128 threshold: 5
    [1.2220247719604558, 1.209619499741031, 1.2193029878230086, 1.212193457892042, 1.2140367012938418]
    [0.9286813928889466, 0.9236568115242315, 0.9255680647810646, 0.9210866058635538, 0.9227774007324817]
    

    Dataset: ../../Datasets/Toys_and_Games.pkl RMSE: 1.2336422702760135 MAE: 0.9371956989147419 Hidden channels: 128 threshold: 5
    [1.2416812169824558, 1.2302356292661727, 1.22613749362471, 1.2372043601022595, 1.2329526514044686]
    [0.9419001685540552, 0.9346277637856153, 0.9330785410550321, 0.9406571672120665, 0.9357148539669405]
    '''
