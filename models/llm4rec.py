...
class llm4rec(nn.Module):
    def __init__(
        ...
        if llm_model == 'llama':
            self.llm_tokenizer = AutoTokenizer.from_pretrained("meta-llama/Llama-3.2-3B", use_fast=False)
            self.llm_model = AutoModelForCausalLM.from_pretrained("meta-llama/Llama-3.2-3B", torch_dtype=torch.float16, device_map=self.device)
        else:
