...
'''
The function random_neq(l, r, s) generates a random integer t in the range [l, r), and 
ensures that this integer does not exist in the set s (i.e., t is not in the set s).
'''
...
Input: User's history of interactions, validation items, and test items.

Output: For each user, it provides:

A sequence of their past interactions.

A positive sample (the item they are expected to interact with next).

Negative samples (items they haven't interacted with).
'''
class SeqDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        '''
        # The next item (target item) the user will interact with, which is the last item in their interaction 
        history ([-1] gives the most recent interaction).
        '''
        nxt = self.user_train[user_id][-1]
        length_idx = self.max_len - 1
        
        # user seq set
        ts = set(self.user_train[user_id])  # Items that the user has interacted with
        for i in reversed(self.user_train[user_id][:-1]):  # Loop through the user's interactions in reverse (excluding the last item)
            seq[length_idx] = i
            pos[length_idx] = nxt
